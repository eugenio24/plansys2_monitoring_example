...
public:
  ...
  {
    if (args.size() != 2) {
      return PredicateSensingResult{PredicateSensingResult::Status::ERROR, "robot_at expects 2 arguments: (robot, room)"};
    }
...
private:
  PredicateSensingResult sense(const std::string& robot, const std::string& room)
  {
    RCLCPP_INFO(rclcpp::get_logger("RobotAtPredicateSensor"),
      "Checking if robot '%s' is in room '%s'", robot.c_str(), room.c_str());

    bool detected = FakeSensingSequences::getInstance()->sensePredicate(symbol_name_, {robot, room});

    if (detected) {
      return PredicateSensingResult{PredicateSensingResult::Status::TRUE, robot + " is at " + room};
    } else {
      return PredicateSensingResult{PredicateSensingResult::Status::FALSE, robot + " is NOT at " + room};
    }
  }
};
